...
export function cloneDefaultPermissions() {
  return clonePermissionsForRole(DEFAULT_ROLE);
}

export function cloneAdminDefaultPermissions() {
  return clonePermissionsForRole('admin');
}
...
export function hasModulePermission(user, moduleKey, action = 'view') {
  if (!user) return false;
  const permissions = normalizePermissions(user.permissions, user.role);
  return Boolean(permissions?.[moduleKey]?.[action]);
}

export function moduleKeyFromView(viewKey) {
  if (viewKey === 'overview') return 'overview';
  if (viewKey === 'people') return 'generalRegistration';
  if (viewKey === 'nhis') return 'nhisRegistration';
  if (viewKey === 'users') return 'userManagement';
